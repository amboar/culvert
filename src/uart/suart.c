			...
			uint16_t base, int sirq)
{
	...
	rc = lpc_writeb(io, ctx->base + UART_LCR,
			(UART_LCR_EPS | UART_LCR_CLS_8));
	if (rc)
		goto cleanup_lpc;

	/* Polled FIFO Mode */
	return lpc_writeb(io, ctx->base + UART_FCR,
			  (UART_FCR_XMIT_RST | UART_FCR_RCVR_RST |
			   UART_FCR_FIFO_EN));
...
int suart_set_baud(struct suart *ctx, int rate)
{
	...
	rc = lpc_writeb(&ctx->io, ctx->base + UART_FCR,
			(UART_FCR_RCVR_RST | UART_FCR_XMIT_RST |
			 UART_FCR_FIFO_EN));
	if (rc)
		return rc;
...
ssize_t suart_read(struct suart *ctx, char *buf, size_t len)
{
	...
	do {
		while (!(rc = lpc_readb(io, ctx->base + UART_LSR, &lsr)) &&
			...
			rc = lpc_readb(io, ctx->base + UART_RBR,
				       (uint8_t *)buf++);
			if (rc)
				return rc;
		}
	...
	while ((rc = epoll_wait(epfd, &epevent, 1, 50)) != -1) {
		if (rc) {
			...
			while (uin_read > 0) {
				...
				uin_write = suart_write(
					ctx, uin_end - uin_write, uin_write);
				if (uin_write < 0)
					return uin_write;

				uout_buf = &_uout_buf[0];
				uout_read = suart_read(ctx, uout_buf,
						       sizeof(_uout_buf));
				if (uout_read < 0)
					return uout_read;

				while (uout_read > 0) {
					uout_wrote = write(uout, uout_buf,
							   uout_read);
					if (uout_wrote == -1)
						return -errno;
			...
			uout_buf = &_uout_buf[0];
			uout_read =
				suart_read(ctx, uout_buf, sizeof(_uout_buf));
			if (uout_read < 0)
				return uout_read;
