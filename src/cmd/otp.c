...
static error_t cmd_otp_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		case 0:
			...
			arguments->region = !strcmp("conf", arg) ?
						    otp_region_conf :
						    otp_region_strap;
			break;
		case 1:
...
static int do_otp(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_otp_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
	...
	if (arguments.read)
		...
		if (arguments.region == otp_region_strap)
			rc = otp_write_strap(otp, arguments.key,
					     arguments.value);
		else
			rc = otp_write_conf(otp, arguments.key,
					    arguments.value);
	}
