...
static char cmd_debug_args_doc[] =
	"<read ADDRESS|write ADDRESS LENGTH> via DRIVER INTERFACE [IP PORT USERNAME PASSWORD]";
...
static error_t cmd_debug_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case ARGP_KEY_END:
		if (state->arg_num < 2)
			argp_error(
				state,
				"Not enough arguments provided. Need at least an address.");

		if (!arguments->ahb_args.read && state->arg_num < 3)
			argp_error(
				state,
				"Write mode detected but either no address or value detected");

		if (arguments->connection.interface == NULL)
			argp_error(state,
				   "Debug interface path must be defined");
		break;
	default:
...
static int do_debug(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_debug_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
	...
	if (!arguments.connection.internet_args) {
		...
		if ((rc = debug_init(debug, arguments.connection.interface)) <
		    0) {
		...
		rc = debug_init(debug, arguments.connection.interface,
				arguments.connection.ip,
				arguments.connection.port,
