...
static struct argp_option cmd_jtag_options[] = {
	{ "controller", 'c', "CTRL", 0,
	  "JTAG controller to use (default: jtag)", 0 },
	{ "port", 'p', "PORT", 0, "Port to listen on (default: 33333)", 0 },
	{ "target-type", 't', "TYPE", 0,
	  "JTAG target to route to (default: arm)", 0 },
	{ 0 },
};
...
static error_t cmd_jtag_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case 'p':
		...
		if (!arguments->listen_port &&
		    arguments->listen_port > UINT16_MAX)
...
static int do_jtag(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_jtag_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
