...
static char cmd_reset_args_doc[] =
	"TARGET WDT [via DRIVER [INTERFACE [IP PORT USERNAME PASSWORD]]]";
...
static error_t cmd_reset_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		case 0:
			...
			if (strcmp("soc", arg))
				argp_error(state, "Invalid reset target '%s'",
					   arg);
			break;
		case 1:
			arguments->wdt = arg;
	...
	case ARGP_KEY_END:
		if (state->arg_num < 2)
			argp_error(
				state,
				"Not enough arguments provided. Need type and watchdog.");
		break;
	default:
...
static int do_reset(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_reset_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
