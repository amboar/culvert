...
static char cmd_probe_args_doc[] =
	"[-l] [-r REQUIREMENT] [via DRIVER [INTERFACE [IP PORT USERNAME PASSWORD]]]";
...
static int do_probe(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_probe_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0) {
		rc = EXIT_FAILURE;
		goto done;
	...
	if (arguments.list_ifaces) {
		...
		rc = (arguments.requirement <= discovered) ? EXIT_SUCCESS :
							     EXIT_FAILURE;
	}
