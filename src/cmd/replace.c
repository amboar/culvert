				     ...
				     struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		case 0:
			if (strcmp(arg, "ram"))
				argp_error(
					state,
					"Invalid type '%s' found. Only 'ram' is allowed",
					arg);
			break;
		case 1:
	...
	case ARGP_KEY_END:
		...
		if (strlen(arguments->mem_match) <
		    strlen(arguments->mem_replace))
			argp_error(
				state,
				"REPLACE length %zd overruns MATCH length %zd, bailing",
				strlen(arguments->mem_match),
				strlen(arguments->mem_replace));
		break;
	default:
...
static int do_replace(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_replace_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
	...
	for (ram_cursor = dram.start; ram_cursor < vram.start;
	     ram_cursor += DUMP_RAM_WIN) {
		logi("Scanning BMC RAM in range 0x%08zx-0x%08zx\n", ram_cursor,
		     ram_cursor + DUMP_RAM_WIN - 1);
