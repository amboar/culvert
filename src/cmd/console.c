...
static struct argp_option cmd_console_options[] = { { 0 } };
	...
	case ARGP_KEY_ARG:
		...
		case 0:
			if (strcmp("uart3", arg))
				argp_error(
					state,
					"Console only supports host on 'uart3'");
		...
		case 1:
			if (strcmp("uart2", arg))
				argp_error(
					state,
					"Console only supports BMC on 'uart2'");
...
static int do_console(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_console_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0) {
		rc = EXIT_FAILURE;
		goto done;
	...
	rc = suart_flush(suart, arguments.user,
			 strlen(arguments.user)); /* username */
	if (rc) {
		errno = -rc;
		perror("suart_flush");
		goto suart_cleanup;
	...
	rc = suart_flush(suart, arguments.pass,
			 strlen(arguments.pass)); /* password */
	if (rc) {
		errno = -rc;
		perror("suart_flush");
		goto suart_cleanup;
