...
static char cmd_p2a_args_doc[] =
	"<vga|bmc> <read ADDRESS|write ADDRESS LENGTH>";
...
static error_t cmd_p2a_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		case 0:
			if (!strcmp("vga", arg))
				...
				argp_error(state, "Invalid PCIe device '%s'",
					   arg);
			break;
		case 1:
			if (strcmp("read", arg) && strcmp("write", arg))
				argp_error(state, "Invalid operation '%s'",
					   arg);
	...
	case ARGP_KEY_END:
		if (state->arg_num < 3)
			argp_error(
				state,
				"Not enough arguments provided. Need the device type, the operation and an address.");
...
static int do_p2a(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_p2a_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
