...
static char cmd_coprocessor_args_doc[] =
	"<run ADDRESS LENGTH|stop> [via DRIVER [INTERFACE [IP PORT USERNAME PASSWORD]]]";
					 ...
					 struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		if (state->arg_num == 1) {
			parse_mem_arg(state, "coprocessor RAM base",
				      &arguments->mem_base, arg);
		} else if (state->arg_num == 2) {
			parse_mem_arg(state, "coprocessor RAM size",
				      &arguments->mem_size, arg);
			if (arguments->mem_size != COPROC_TOTAL_MEM_SIZE)
				argp_error(
					state,
					"Only 32M memory is currently supported for the coprocessor");
		}
	...
	case ARGP_KEY_END:
		if (arguments->mode == run && state->arg_num < 3)
			argp_error(state,
				   "Not enough arguments for run mode...");
		if (arguments->mode == stop && state->arg_num < 1)
			argp_error(state,
				   "Not enough arguments for stop mode...");
		break;
	default:
...
static int cmd_coprocessor_run(struct cmd_coprocessor_args *arguments)
{
	...
	if (((arguments->mem_base + arguments->mem_size) & UINT32_MAX) <
	    arguments->mem_base) {
	...
	/* 2. */
	if ((rc = scu_writel(scu, SCU_COPROC_CTRL,
			     SCU_COPROC_CTRL_RESET_ASSERT)) < 0) {
	...
	/* 3. */
	if ((src = soc_siphon_in(soc, arguments->mem_base, arguments->mem_size,
				 STDIN_FILENO)) < 0) {
	...
	if (scu_writel(scu, SCU_COPROC_MEM_BASE, arguments->mem_base) ||
	    ...
	    scu_writel(scu, SCU_COPROC_DMEM_LIMIT,
		       arguments->mem_base + arguments->mem_size) ||
	    /* 7. */
	    scu_writel(scu, SCU_COPROC_CACHE_RANGE,
		       SCU_COPROC_CACHE_1ST_16MB_EN) ||
	    /* 8. */
	...
	if (usleep(1000) == -1) {
		loge("Coprocessor reset post-delay failed: %s",
		     strerror(errno));
		rc = EXIT_FAILURE;
		goto cleanup_scu;
	}
...
static int do_coprocessor(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_coprocessor_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0) {
		rc = EXIT_FAILURE;
		goto done;
