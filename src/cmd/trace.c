...
static error_t cmd_trace_parse_opt(int key, char *arg, struct argp_state *state)
{
	...
	case ARGP_KEY_ARG:
		...
		case 1:
			...
			if (arguments->width != 1 && arguments->width != 2 &&
			    arguments->width != 4)
				argp_error(state, "Invalid access size '%lu'",
					   arguments->width);
			break;
		case 2:
			if (!strcmp(arg, "read"))
	...
	case ARGP_KEY_END:
		if (arguments->address & (arguments->width - 1))
			argp_error(
				state,
				"Listening address must be aligned to the access size");
		break;
	default:
...
static int do_trace(int argc, char **argv)
{
	...
	rc = argp_parse(&cmd_trace_argp, argc, argv, ARGP_IN_ORDER, 0,
			&arguments);
	if (rc != 0)
		return rc;
	...
	if ((rc = trace_start(trace, arguments.address, arguments.width,
			      arguments.mode))) {
