...
static int otp_set_soak(struct otp *otp, unsigned int soak)
{
	...
	if ((rc = otp_write_reg(otp, 0x3000, otp->soak_parameters[soak][0])) <
	    0)
		return rc;

	if ((rc = otp_write_reg(otp, 0x5000, otp->soak_parameters[soak][1])) <
	    0)
		return rc;

	if ((rc = otp_write_reg(otp, 0x1000, otp->soak_parameters[soak][2])) <
	    0)
		return rc;
...
int otp_read(struct otp *otp, enum otp_region reg)
{
	...
	if (reg == otp_region_strap) {
		...
		for (i = 0; i < 6; ++i) {
			...
			if ((rc = otp_read_config(otp, o + 1, &strap[i][1])) <
			    0)
				goto done;
