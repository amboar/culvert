...
void bridgectl_log_status(struct bridgectl *ctx, int fd,
			  enum bridge_mode status)
{
	int rc;

	log_highlight(fd, bridge_mode_colour[status], "%s:\t%s",
		      bridgectl_name(ctx), bridge_mode_description[status]);
