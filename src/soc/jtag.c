...
static const struct jtag_ops ast2400_jtag_ops = { .release =
							  ast2400_jtag_release,
...
static int jtag_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if ((rc = jtag_writel(ctx, AST_JTAG_EC,
			      AST_JTAG_EC_ENG_EN | AST_JTAG_EC_ENG_OUT_EN)) <
	    0) {
		goto cleanup_ctx;
	}
	...
	if ((rc = jtag_writel(ctx, AST_JTAG_SW_MODE, AST_JTAG_SW_MODE_EN)) <
	    0) {
		goto cleanup_ctx;
	}
...
{
	struct jtag *ctx =
		soc_driver_get_drvdata_by_name(soc, &jtag_driver, name);
	if (ctx) {
		ctx->refcnt += 1;
	}
	return ctx;
