...
static const struct bridge_gate_desc ast2500_bridge_gates[] = {
	[AST2500_DEBUG_UART_GATE] = { .reg = AST2500_SCU_MISC,
				      .mask = AST2500_SCU_MISC_UART_DBG },
};
...
static const struct bridge_gate_desc ast2600_bridge_gates[] = {
	[AST2600_DEBUG_UART1_GATE] = { .reg = AST2600_SCU_DBGCTL2,
				       .mask = AST2600_SCU_DBGCTL2_UART1_DBG },
	[AST2600_DEBUG_UART5_GATE] = { .reg = AST2600_SCU_DBGCTL1,
				       .mask = AST2600_SCU_DBGCTL1_UART5_DBG },
	[AST2600_P2A_GATE] = { .reg = AST2600_SCU_DBGCTL1,
			       .mask = AST2600_SCU_DBGCTL1_P2A },
	[AST2600_XDMA_GATE] = { .reg = AST2600_SCU_DBGCTL1,
				.mask = AST2600_SCU_DBGCTL1_XDMA },
	[AST2600_XDMA_VGA_GATE] = { .reg = AST2600_SCU_DBGCTL1,
				    .mask = AST2600_SCU_DBGCTL1_XDMA_VGA },
	[AST2600_ILPC_GATE] = { .reg = AST2600_SCU_DBGCTL2,
				.mask = AST2600_SCU_DBGCTL2_ILPC },
};
...
static const struct soc_device_id bridges_matches[] = {
	{ .compatible = "aspeed,ast2500-bridge-controller",
	  .data = &ast2500_bridge_pdata },
	{ .compatible = "aspeed,ast2600-bridge-controller",
	  .data = &ast2600_bridge_pdata },
	{},
};
...
int bridges_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	ctx->soc = soc;
	ctx->pdata =
		soc_device_get_match_data(soc, bridges_matches, &dev->node);
...
static struct bridges *bridges_get_by_prop(struct soc *soc,
...
struct bridges *bridges_get_by_device(struct soc *soc, struct soc_device *dev)
{
	...
	cells = fdt_getprop(soc->fdt.start, dev->node.offset, "bridge-gates",
			    &len);
	if (len < 0) {
...
int bridges_device_get_gate_by_index(struct soc *soc, struct soc_device *dev,
				     int index, struct bridges **bridges,
				     int *gate)
{
	const uint32_t *cells;
	int len;
...
int bridges_device_get_gate(struct soc *soc, struct soc_device *dev,
			    struct bridges **bridges, int *gate)
{
	return bridges_device_get_gate_by_index(soc, dev, 0, bridges, gate);
}

int bridges_device_get_gate_by_name(struct soc *soc, struct soc_device *dev,
				    const char *name, struct bridges **bridges,
				    int *gate)
{
	int idx;
	int rc;

	idx = fdt_stringlist_search(soc->fdt.start, dev->node.offset,
				    "bridge-gate-names", name);
	if (idx < 0) {
...
static int bridges_configure(struct bridges *ctx, int bridge, bool enable)
{
	...
	if ((rc = soc_readl(ctx->soc, phys, &val)) < 0) {
		loge("Failed to read bridge control register 0x%" PRIx32
		     ": %d\n",
		     phys, rc);
		return rc;
	}
	...
	if ((rc = soc_writel(ctx->soc, phys, val)) < 0) {
		loge("Failed to write 0x%" PRIx32
		     " to bridge control register 0x%" PRIx32 ": %d\n",
		     phys, rc);
		return rc;
	}
