...
static int trace_style(int width, int offset)
{
	...
	case 2:
		...
		return offset == 0 ? AHBC_BCR_CSR_POLL_DATA_2_0 :
		...
		enum trace_mode mode)
{
	...
	logi("Zeroing trace buffer [%p - %p]\n", ctx->sram.start,
	     ctx->sram.start + ctx->sram.length);
...
int trace_dump(struct trace *ctx, int outfd)
{
	...
	buf_len = (csr & AHBC_BCR_CSR_BUF_LEN_MASK) >>
		  AHBC_BCR_CSR_BUF_LEN_SHIFT;
	...
	if (wrapped) {
		...
		logd("Ring buffer has wrapped, dumping trace buffer from write pointer at 0x%" PRIx32
		     " for %zu\n",
		     buf, len);
...
static int trace_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	rc = soc_device_get_memory_region_named(soc, &dev->node, "trace-buffer",
						&ctx->sram);
	if (rc < 0) {
		goto cleanup_ctx;
	}
...
static const struct soc_driver trace_driver = { .name = "trace",
						.matches = ahbc_match,
						.init = trace_driver_init,
						.destroy =
							trace_driver_destroy };
