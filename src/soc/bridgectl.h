...
static inline int bridgectl_enforce(struct bridgectl *ctx,
				    enum bridge_mode mode)
{
	return ctx->ops->enforce(ctx, mode);
}

static inline int bridgectl_status(struct bridgectl *ctx,
				   enum bridge_mode *mode)
{
	return ctx->ops->status(ctx, mode);
}

static inline int bridgectl_report(struct bridgectl *ctx, int fd,
				   enum bridge_mode *mode)
{
