...
int sioctl_decode_configure(struct sioctl *ctx, const enum sioctl_decode mode)
{
	...
	if (mode == sioctl_decode_disable) {
		return strap_set(ctx->strap, ctx->pdata->reg,
				 ctx->pdata->disable, ctx->pdata->disable);
	}

	if (mode == sioctl_decode_4e) {
		rc = strap_set(ctx->strap, ctx->pdata->reg, ctx->pdata->select,
			       ctx->pdata->select);
		if (rc < 0) {
			return rc;
		}
		...
		rc = strap_clear(ctx->strap, ctx->pdata->reg,
				 ctx->pdata->select, ctx->pdata->select);
		if (rc < 0) {
			return rc;
		}
	}

	return strap_clear(ctx->strap, ctx->pdata->reg, ctx->pdata->disable,
			   ctx->pdata->disable);
}
...
int sioctl_decode_status(struct sioctl *ctx, enum sioctl_decode *status)
{
	...
	*status = (strap & ctx->pdata->select) ? sioctl_decode_4e :
						 sioctl_decode_2e;
...
static int sioctl_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if (!(ctx->pdata = soc_device_get_match_data(soc, sioctl_matches,
						     &dev->node))) {
