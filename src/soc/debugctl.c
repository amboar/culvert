...
static int ast2500_debugctl_report(struct bridgectl *bridge, int fd,
				   enum bridge_mode *mode)
{
	...
	if ((rc = soc_readl(ctx->soc, ctx->region.start + SCU_STRAP, &strap)) <
	    0) {
...
static int ast2600_debugctl_report(struct bridgectl *bridge, int fd,
				   enum bridge_mode *mode)
{
	struct debugctl *ctx = to_debugctl(bridge);
	int rc;
...
static int debugctl_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if ((rc = bridges_device_get_gate(soc, dev, &ctx->bridges, &ctx->id)) <
	    0) {
		goto cleanup_ctx;
	}
	...
	if ((rc = soc_bridge_controller_register(
		     soc, debugctl_as_bridgectl(ctx), ops)) < 0) {
		goto cleanup_drvdata;
	}
