...
static inline ssize_t soc_read(struct soc *ctx, uint32_t phys, void *buf,
			       size_t len)
{
	return ahb_read(ctx->ahb, phys, buf, len);
}

static inline ssize_t soc_write(struct soc *ctx, uint32_t phys, const void *buf,
				size_t len)
{
...
static inline ssize_t soc_siphon_out(struct soc *ctx, uint32_t phys, size_t len,
				     int outfd)
{
	return ahb_siphon_out(ctx->ahb, phys, len, outfd);
}

static inline ssize_t soc_siphon_in(struct soc *ctx, uint32_t phys,
				    ssize_t length, int infd)
{
...
static inline int soc_device_get_memory(struct soc *ctx,
					const struct soc_device_node *dn,
					struct soc_region *region)
{
...
void soc_bridge_controller_unregister(struct soc *soc,
